#!/bin/bash
echo "🏛️ IMPERIAL OMEGA DASHBOARD STATUS"
echo "================================"
echo ""

# Check Node-RED
if curl -s http://127.0.0.1:1880/ > /dev/null 2>&1; then
    echo "✅ Node-RED: Running on port 1880"
    echo "   📊 Dashboard: http://localhost:1880/ui"
    echo "   ✏️ Editor: http://localhost:1880"
else
    echo "❌ Node-RED: Not running"
fi

# Check API endpoint
echo ""
if curl -s -X POST http://127.0.0.1:1880/village_data -H "Content-Type: application/json" -d '{"test":"ping"}' > /dev/null 2>&1; then
    echo "✅ API Endpoint: /village_data is active"
else
    echo "❌ API Endpoint: Not responding"
fi

# Check data feeder
echo ""
if pgrep -f "data_feeder.py" > /dev/null; then
    echo "✅ Data feeder: Running"
else
    echo "❌ Data feeder: Not running"
fi

# Check flows
echo ""
if [ -f ~/.node-red/flows.json ]; then
    FLOW_COUNT=$(grep -c '"id":' ~/.node-red/flows.json)
    echo "✅ Flows: $FLOW_COUNT nodes loaded"
else
    echo "❌ Flows: flows.json not found"
fi

# Show recent data from feeder log
echo ""
if [ -f ~/imperial_network/feeder.log ]; then
    echo "📊 Recent data updates:"
    tail -3 ~/imperial_network/feeder.log 2>/dev/null | grep "Data sent" || echo "   No recent updates"
fi
